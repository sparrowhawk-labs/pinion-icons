# pinion-icons — Full Icon Index
Searchable list of every icon name available via `<x-i type="{name}" />`. Default library is Solar (1,234 concept names, each with 6 style variants). See [REFERENCE.md](./REFERENCE.md) for the variant matrix and the virtual `emoji` / `pixel` variants that pull from Fluent Emoji and Pixelarticons.
**For AI agents**: this file is grep-friendly — `rg "^- cart"` to find every cart-* icon, `rg -F "hamburger"` to spot the menu glyph, etc. Names are kebab-case throughout.
## How to use
```blade
<x-i type="hamburger-menu" />                 {{-- Solar bold-duotone (default) --}}
<x-i type="hamburger-menu" variant="linear" /> {{-- Solar linear --}}
<x-i type="heart" variant="emoji" />          {{-- Fluent Emoji red heart --}}
<x-i type="heart" variant="pixel" />          {{-- Pixelarticons heart --}}
<x-i type="close" library="solar-extra" />    {{-- plain ✕ glyph (no surrounding shape) --}}
```
## solar-extra — plain glyphs (no shape)
For ✕ / ✓ / ＋ / − without Solar's surrounding circle or square, use `library="solar-extra"`. Same 6 style variants as Solar.
- `check`
- `close`
- `minus`
- `plus`
## Solar — common prefixes (quick discovery)
Solar names are prefix-grouped; sorting alphabetically clusters related icons. Common entry points:
| Prefix | Use case | Example names |
|---|---|---|
| `add-*` | Add / create actions | `add-circle`, `add-folder`, `add-square` |
| `alt-arrow-*` / `arrow-*` / `round-arrow-*` | Directional arrows | `arrow-right`, `alt-arrow-down`, `round-arrow-up` |
| `bag-*` / `cart-*` | Commerce | `bag-smile`, `cart-large`, `cart-cross` |
| `bell-*` | Notifications | `bell`, `bell-bing`, `bell-off` |
| `calendar-*` | Dates | `calendar`, `calendar-add`, `calendar-minimalistic` |
| `card-*` / `wallet-*` | Payments | `card`, `wallet-money`, `card-recive` |
| `chart-*` / `graph-*` | Data viz | `chart`, `graph-up`, `chart-square` |
| `chat-*` / `message-*` | Messaging | `chat-round`, `message-square`, `chat-dots` |
| `check-*` | Confirmation | `check-circle`, `check-square`, `checklist` |
| `close-*` | Dismiss | `close-circle`, `close-square` (plain X: `solar-extra/close`) |
| `download-*` / `upload-*` | Transfer | `download`, `download-minimalistic`, `upload-square` |
| `eye-*` | Visibility | `eye`, `eye-closed`, `eye-scan` |
| `file-*` / `folder-*` / `document-*` | Storage | `file-text`, `folder`, `document-add` |
| `heart-*` / `like-*` | Affection | `heart`, `heart-broken`, `like` |
| `home-*` / `home2-*` | Home | `home`, `home-2`, `home-smile` |
| `lock-*` / `unlock-*` | Security | `lock`, `lock-password`, `unlock-keyhole` |
| `menu-*` / `hamburger-*` | Navigation | `hamburger-menu`, `menu-dots`, `menu-dots-square` |
| `pen-*` / `pencil-*` | Edit | `pen`, `pen-new-square`, `pencil` |
| `phone-*` / `call-*` | Phone | `phone`, `call-medicine`, `phone-calling` |
| `play-*` / `pause-*` / `stop-*` | Media | `play`, `pause`, `stop-circle` |
| `search-*` / `magnifer-*` | Search | `search`, `magnifer`, `magnifer-zoom-in` |
| `settings-*` | Settings | `settings`, `settings-minimalistic` |
| `share-*` | Share | `share`, `share-circle` |
| `star-*` | Favourites | `star`, `star-circle`, `star-fall` |
| `trash-*` | Delete | `trash-bin-minimalistic`, `trash-bin-2`, `trash-bin-trash` |
| `user-*` / `users-*` | People | `user`, `users-group-rounded`, `user-plus` |

## Solar — full list (1,234 names)
### 0-9
- `4k`
...
## Virtual variants
- `variant="emoji"` → renders the matching Fluent Emoji (2,397 native names also addressable directly).
- `variant="pixel"` → renders the matching Pixelarticons (1,632 native names also addressable directly).

Many Solar names map automatically via the bundled `solar-variant-mapping.json` (e.g. `accessibility` → emoji `wheelchair_symbol`, pixel `human`). Falls back to a default question-mark glyph if no mapping exists.
